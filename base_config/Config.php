<?php 
error_reporting(E_ALL);
ini_set("display_errors", 1);
date_default_timezone_set("Asia/Calcutta");
define('user' , 'root') ; 
define('password' ,'') ; 
define('server' , 'localhost') ; 
define('dbName' , 'hotel');
define('RAZOR_PAY_API_KEY' , 'RAZOR KEY API');
define('RAZOR_PAY_SECRET_TOKEN' , 'RAZOR PAY SECRET TOKEN');
define('TOTAL_NUMBER_WORKFORCE' , 2);
define('MSG_API_KEY' , 'API KEY') ;
define('OTP_TEMPLATE' , 'TEMPLATE') ;
/**Pay u money details */
define('PAY_U_MERCH_KEY' , 'MERCH KEY');
define('PAY_U_MERCH_SALT' , 'SALT');
define('PAY_U_AUTH_HEADER' , 'HEADER+vmE=');
define ("SECRETKEY", "SECRETKEYFOD");
define("PAY_U_MONEY_URL" , "https://sandboxsecure.payu.in/_payment");



?>